...
- Saat ubah/tambah node (file di scripts) serta ubah apapun di dalam folder shell_simulation itu wajib jalankan `colcon build` ya! (karena ini penting agar semua script ini bisa di executable oleh ros2 dan disini jga biar dia inisialisasi package yang kamu setting di package.xml)
- Kalo abis trial error kan mobil suka ngaco tuh jalannya nah kalo mau restart tekan `Ctrl+C` aja di terminal yang run carla_bridge. Kalau udah stop baru run lagi pake `ros2 launch carla_shell_bridge main.launch.py`
- Gunakan Rviz untuk lihat hasil dari topic/publisher yang kalian buat :
  Konsep Dasar
    Publisher (Penerbit)
...
- Ada GPT gunakan yang baik dan benar (sekalian belajar!) 
